...
impl<'w> VM<'w> {
    fn step(&mut self, code: &bytecode::Code) -> Result<Option<i64>> {
        let ip = self.ip();
    ...
    pub fn run(&mut self, code: &bytecode::Code, main_key: bytecode::FuncKey) -> Result<i64> {
        self.seed(main_key);
        self.run_to_end(code)
    }

    fn seed(&mut self, main_key: bytecode::FuncKey) {
        // We assert a fresh VM, which is probably bad design
    ...
    fn run_to_end(&mut self, code: &bytecode::Code) -> Result<i64> {
        loop {
