...
use debug2::{Debug, Formatter};

// TODO: Figure out a way to test this

// Keep in sync with debug_impl.rs

impl Debug for Code<'_, '_> {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        let mut s = f.debug_struct("Code");
        for (name, f) in self.fns.iter().map(|(_, f)| (f.name, FnDbg(f, self))) {
            s.field(name, &f);
        }
        s.finish()
    }
...
struct FnDbg<'a>(&'a Func<'a, 'a>, &'a Code<'a, 'a>);

impl Debug for FnDbg<'_> {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        // TODO: Print FuncKey's as their name
        f.debug_list()
            .entries(self.0.code.iter().map(|i| InstrDebug(i, self.1)))
            .finish()
    }
}
